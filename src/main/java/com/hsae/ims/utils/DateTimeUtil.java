...
public class DateTimeUtil {
   public final static int TIME_DAY_MILLISECOND = 86400000;
    ...
    private final static String TIME_HOUR_MINUTE = "HH:mm";
...
//    }
     ...
     * 				15:23
     */
    public static Date getFormatTime(String currTime){
    	return getFormatTime(currTime, TIME_HOUR_MINUTE);
    }
     ...
     *         定义，如HH:mm，如08:30
     */
    public static Date getFormatTime(String currTime,String format){
    	if(currTime == null){
    		return null;
    	}
    	SimpleDateFormat dtFormatdB = null;
    	try {
            dtFormatdB = new SimpleDateFormat(format);
            return dtFormatdB.parse(currTime);
        } catch (Exception e) {
            dtFormatdB = new SimpleDateFormat(TIME_HOUR_MINUTE);
            try {
                return dtFormatdB.parse(currTime);
            } catch (Exception ex) {
            }
    ...
    /**
     * 获得两个Date型日期之间相差的毫秒数（第2个减第1个）
     *
     * @param Date
     *                first, Date second
     * @return int 相差的天数
     */
    public static int getDaysBetweenMils(Date first, Date second) {
        Long mils = second.getTime() - first.getTime() ;
        return mils.intValue();
    }
    ...
    public static List<Date> getDaysListBetweenDates(String first, String second) {
        List<Date> dateList = new ArrayList<Date>();
        Date d1 = getFormatDateTime(first,DATE_FORMAT);
        Date d2 = getFormatDateTime(second,DATE_FORMAT);
        if(d1.compareTo(d2)>0) {
    ...
    public static Date getFormatMonth(String currDate){
    	return getFormatDate(currDate, MONTH_FORMAT);
    }
...
//        }
    ...
    public static boolean isConfilct(Date begintimeOne, Date endtimeOne, Date begintimeTwo,
    		Date endtimeTwo) {
    	Date beginOne = getFormatDateTimeToTime(begintimeOne);
        Date endOne = getFormatDateTimeToTime(endtimeOne);
        Date beginTwo = getFormatDateTimeToTime(begintimeTwo);
        Date endTwo = getFormatDateTimeToTime(endtimeTwo);
        if ((beginOne.compareTo(beginTwo) <= 0 && endOne.compareTo(beginTwo) >= 0)
                || (beginOne.compareTo(endTwo) <= 0 && endOne.compareTo(endTwo) >= 0)
                || (beginTwo.compareTo(beginOne) <= 0 && endTwo.compareTo(beginOne) >= 0)
                || (beginTwo.compareTo(endOne) <= 0 && endTwo.compareTo(endOne) >= 0)) {
	...
	public static int minus(String time1,String time2){
		...
		calendar1.setTime(DateTimeUtil.getFormatTime(time1));
		Calendar calendar2 = Calendar.getInstance();
		calendar2.setTime(DateTimeUtil.getFormatTime(time2));
		long min = 0;
		if(calendar1.compareTo(calendar2)>=0){
			min = calendar1.getTimeInMillis() - calendar2.getTimeInMillis();
		}else if(calendar1.compareTo(calendar2)<0){
			min = calendar2.getTimeInMillis() - calendar1.getTimeInMillis();
		}
